...
public:
    explicit TCoreAudioDriver(TAudioDevice* device);
    ~TCoreAudioDriver() override;

    int _read(nframes_t nframes) override;
    int _write(nframes_t nframes) override;
    int _run_cycle() override { return 1; }
    int setup(bool capture = true, bool playback = true, const QString& device = "none");
    int attach() override;
    int start() override;
    int stop() override;
    ...
    bool runs_in_blocking_mode() const override { return false; }
