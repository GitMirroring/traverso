...
TAudioClipGroup::TAudioClipGroup()
{
    m_location = new TLocation(this);
}

TAudioClipGroup::TAudioClipGroup(QList< TAudioClip * > clips)
{
    m_location = new TLocation(this);
    m_clips = clips;
    update_state();
...
void TAudioClipGroup::add_clip(TAudioClip * clip)
{
    m_clips.append(clip);
    update_state();
}

void TAudioClipGroup::set_clips(QList< TAudioClip * > clips)
{
    m_clips = clips;
    update_state();
}

void TAudioClipGroup::move_to(int trackIndex, const TTimeRef &location)
{
    PENTER;
    int trackIndexDelta = trackIndex - m_topTrackIndex;

    for(TAudioClip* clip : m_clips) {
        if (trackIndexDelta != 0) {
            TAudioTrack* track = clip->get_sheet()->get_audio_track_for_index(clip->get_track()->get_sort_index() + trackIndexDelta);
            if (track) {
...
void TAudioClipGroup::update_state()
{
    if (m_clips.isEmpty()) {
        return;
    }
...
void TAudioClipGroup::set_snappable(bool snap)
{
    foreach(TAudioClip* clip, m_clips) {
...
void TAudioClipGroup::set_as_moving(bool move)
{
    foreach(TAudioClip* clip, m_clips) {
...
QList<TAudioClip*> TAudioClipGroup::copy_clips()
{
    QList<TAudioClip*> newclips;
...
void TAudioClipGroup::add_all_clips_to_tracks()
{
...
void TAudioClipGroup::remove_all_clips_from_tracks()
{
    for(TAudioClip* clip : m_clips) {
        TAudioClipAddRemoveSpec spec;
        spec.set_clip(clip);
...
int TAudioClipGroup::check_valid_track_index_delta(int delta)
{
    if (m_clips.isEmpty()) {
        return 0;
    }
