...
{
    public:
        TileHash() : tileX(0), zoom(0), height(0), devicePixelRatio(0), selected(0), hover(0) {}
        TileHash(qint32 tileX, int height, int zoom, int devicePixelRatio, bool selected, bool hover) : 
            tileX(tileX), zoom(zoom), height(height), devicePixelRatio(devicePixelRatio), selected(selected), hover(hover) {}
        TileHash(const TileHash& other) = default;
        TileHash& operator=(const TileHash& other) = default;
...
class TileRef
{
    public:
        TileRef() : clip(0) {}
        TileRef(TAudioClip *clip, TileHash hash) : clip(clip), hash(hash) {}
        TileRef(const TileRef& other) = default;
        TileRef& operator=(const TileRef& other) = default;
        TAudioClip *clip;
        TileHash hash;
};
...
public:
    ...
    ClipTile* find(TAudioClip* clip, TileHash hash);
    ClipTile& insert(TAudioClip* clip, TileHash hash, quint64 start, const QSize& size);
    void invalidate_clip(TAudioClip* clip);
    void invalidate_clip_range(TAudioClip* clip, int zoom, qreal left, qreal right);
    void invalidate_all();

private:
    QHash<TAudioClip*, QHash<TileHash, ClipTile> > m_tiles;
