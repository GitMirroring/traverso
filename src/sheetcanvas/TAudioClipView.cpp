    ...
    , m_clip(clip)
{
    ...
    connect(m_clip, &TAudioClip::stateChanged, this, [this](){update();});
...
void TAudioClipView::draw_peaks(QPainter* p, qreal xstart, int pixelcount)
{
    ...
    for (uint chan=0; chan < channels; ++chan) {
        ...
        if (availpeaks == TPeak::NO_PEAK_FILE) {
        ...
        if (microView) {
            ...
            scaleFactor = ( (float) height * 0.90 / (TPeak::MAX_DB_VALUE * 2)) * m_clip->get_gain() * curveDefaultValue;
            ...
            if (m_classicView) {
                ...
                scaleFactor =  (float) height * 0.95 * m_clip->get_gain() / TPeak::MAX_DB_VALUE * curveDefaultValue;
